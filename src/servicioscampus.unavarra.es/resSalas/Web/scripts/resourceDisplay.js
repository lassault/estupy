function ResourceDisplay(opts) {
	...
	// UPNA Capturar respuesta ajax. Copiado del dashboard
	
    var ShowAjaxResponse = function (data) {

    	$('.blockUI').css('cursor', 'default');

		
		var mens = document.getElementById("mensajeRes");
		mens.innerHTML = data;		
		console.log(mens);
		
		
		$.blockUI({ message: $('#resultado'), css: { width: '275px' } }); 
    ...
    ResourceDisplay.prototype.initDisplay = function (opts) {
        ...
        elements.placeholder.on('click', '#checkin', function (e) {
            ...
            var afterCheckin = function (data) {			   
                refreshResource();
                hideWait();
				...
				if (data.success.toString()=="true")
				 {
				  ShowAjaxResponse("Check-In Realizado");
				 }
				 else
				 {
				  ShowAjaxResponse("El Check-In no pudo hacerse\n"+data.errors);
				 }
				console.log(data);
			};
        ...
        elements.placeholder.on('click', '#checkout', function (e) {
            e.preventDefault();
            e.stopPropagation();

            var beforeCheckout = function () {
                showWait();
            };

			
            var afterCheckout = function (data) {			   
                refreshResource();
                hideWait();
				
				if (data.success.toString()=="true")
				 {
				  ShowAjaxResponse("Check-out Realizado");
				 }
				 else
				 {
				  ShowAjaxResponse("El Check-out no pudo hacerse\n"+data.errors);
				 }
				console.log(data);
			};			
			

            ajaxPost($('#formCheckout'), null, beforeCheckout, afterCheckout);
        });		
        ...
        function refreshResource() {
            ...
            ajaxGet(url, null, function (data) {
				...
				ConfigureAsyncForm(formCheckout, null, afterCheckout, null, {
                    onBeforeSubmit: showWait,
                    onBeforeSerialize: beforeCheckout
                });
			...
			function beforeCheckout() {
                $('#referenceNumber').val($('td[data-checkout="1"]').attr('data-refnum'));
            }
